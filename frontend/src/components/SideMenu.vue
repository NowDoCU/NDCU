...
export default {
  ...
  methods: {
    ...
    controlModal: function(num, con) {
      if (num === 1) { // 회원가입모달 대상이고
        if (con === 'open') { // 열라는 명령이면
    ...
    onFocused: function (type) { // 인풋창에 포커스됐을때 css적용위해
    ...
    onBlured: function (type) { // 인풋창에서 포키스아웃됐을 때 css 컨트롤위해
      ...
      if ( type === 'email' ) {
        if (!EmailValidator.validate(targetInput)) { // email 형식아니면
